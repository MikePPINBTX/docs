...
surfaces: [web]
category: features
related: [features-contracts, admin-operator-comms-cockpit]
status: stable
...
The workflow engine supports the following user-configurable trigger events. (Internal trigger names like `report.html` and `report.pdf` are render lifecycle events and are not configurable as workflow steps.)

### Lead & quote triggers

| Trigger | When it fires |
|---------|--------------|
| `quote.submitted` | A lead requests a quote via the public booking flow |
| `quote.sent` | An operator manually sends a quote from the Quotes screen |
| `quote.accepted` | The client accepts a quote (single or multiple primary services) |
| `booking.confirmed` | Quote acceptance has produced a confirmed booking |
...
### Inspection lifecycle triggers

| Trigger | When it fires |
|---------|--------------|
| `inspection.scheduled` | Inspection date/time is first set on the order |
| `inspection.rescheduled` | The inspection's date or time is changed |
| `inspection.reminder` | N hours/days before the scheduled date (use negative offset) |
| `inspection.completed` | Inspection marked complete by inspector or operator |

### Payment & report triggers

| Trigger | When it fires |
|---------|--------------|
| `payment.received` | Payment is recorded (manual or Stripe) |
| `report.published` | Report is published from Report Workspace |
| `report.ancillary_uploaded` | An ancillary report (e.g. Sewer Scope PDF) is uploaded to the order |
| `report.delivered` | Report has been delivered to the configured recipients |

### Order closure triggers
